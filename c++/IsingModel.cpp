...
IsingModel::IsingModel(int size, double t) :	rng(rd), uni(0, size), r(0, 1) {
	this->size = size;
	this->t = t;
...
int IsingModel::get_site(int x, int y) {
	...
	int x = x % this->size;
	int y = y % this->size;
	int result = this->lattice[x + y * this->size];
	return result * 2 - 1;
}
...
void IsingModel::flip_site(int x, int y) {
	int x = x % this->size;
	int y = y % this->size;
	this->lattice[x + y * this->size] = !this->lattice[x + y * this->size];
}
...
void IsingModel::start(int max_iterations, int sample_every, int delay) {
	...
	this->magnetization[0] = this->calc_energy();
	int i = 1;
...
int IsingModel::metropolis_step(int i) {
	...
	if (deltaE <= 0 || r <= p) {
		...
		this->magnetization[i] = this->magnetization[i - 1] + 2 * this->get_site(randX, randY) / (this->size * this->size);
	}
	else {
